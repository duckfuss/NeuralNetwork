...
for i in range(length):
    if i % (length/10) == 0:
        ...
        print((duck.performanceTest(1000)/1000) * 100,"\tpercent correct\n")
    duck.trainNetwork(10)

print("\n\nDONE-----------\n\n")
print(duck.performanceTest(10000)/10000 * 100,"\tpercent correct\n")
'''
desired = np.zeros(duck.layerData[-1]).reshape(-1,1)
desired[duck.tData.trainLab[9]] = 1
# calculate actual output
output = duck.compute(9, verbose=True) # correct answer is 3
# calculate cost
cost = duck.cost(output,desired, verbose=True)'''
